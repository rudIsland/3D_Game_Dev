...
public class PlayerTargetLookState : PlayerBaseState
{
    ...
    private Vector3 CalculateMove(float deltaTime)
    {
        ...
        {
            Vector3 targetDir = stateMachine.targeter.currentTarget.transform.position
        ...
        // 이동 방향은 카메라 기준이 아니라 "캐릭터의 로컬 기준"
        Vector3 move =
            stateMachine.transform.forward * input.y +
            stateMachine.transform.right * input.x;
