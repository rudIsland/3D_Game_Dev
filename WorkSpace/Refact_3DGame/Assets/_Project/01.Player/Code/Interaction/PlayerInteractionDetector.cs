...
namespace Characters.Player.Interaction
{
    ...
    internal sealed class PlayerInteractionDetector
    {
        ...
        private PlayerInteractionGuide currentGuide;
        ...
        {
            IPlayerInteractable nextTarget = FindNearbyInteractable(
                player,
                out PlayerInteractionGuide nextGuide);
            ...
            currentGuide = nextGuide;
            CurrentGuideChanged?.Invoke(currentGuide);
        }
        ...
        public void ClearCurrentTarget()
        {
            ...
            currentTarget = null;
            currentGuide = PlayerInteractionGuide.Hidden;
            CurrentGuideChanged?.Invoke(currentGuide);
        }

        private IPlayerInteractable FindNearbyInteractable(
            PlayerController player,
            out PlayerInteractionGuide guide)
        {
            guide = PlayerInteractionGuide.Hidden;
            if (!CanDetect(player))
            {
                return null;
            }
            ...
            for (int index = 0; index < detectedCount; index++)
            {
                ...
                PlayerInteractionGuide candidateGuide =
                    interactable.GetInteractionGuide(player);
                if (!candidateGuide.IsVisible ||
