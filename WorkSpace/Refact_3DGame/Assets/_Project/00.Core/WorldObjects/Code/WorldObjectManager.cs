...
namespace World
{
    ...
    public sealed class WorldObjectManager : MonoBehaviour
    {
        ...
        private readonly Dictionary<int, HashSet<SpawnSettings>> scenePools = new Dictionary<int, HashSet<SpawnSettings>>();
        ...
        private void Awake()
        {
            SceneManager.sceneUnloaded += RemoveScenePools;
        ...
        private void Update()
        {
            if (!IsPaused) TickActiveObjects(Time.deltaTime);
        }
        ...
        private bool AddPool(SpawnSettings settings, bool warmUp = true, Scene ownerScene = default)
        {
            if (settings == null ||
            ...
            if (ownerScene.IsValid()) SceneManager.MoveGameObjectToScene(poolContainer.gameObject, ownerScene);
            else poolContainer.SetParent(transform, false);
        ...
        // 소환 지점이 자기 씬의 풀을 등록한다. 같은 씬의 여러 Zone은 같은 풀을 사용한다.
        public void RegisterScenePool(Scene scene, SpawnSettings settings)
        {
            if (isTicking || isShuttingDown || !scene.IsValid() || settings == null || settings.Prefab == null)
                throw new InvalidOperationException("씬의 소환 설정을 등록할 수 없습니다.");
            if (scenePools.TryGetValue(scene.handle, out HashSet<SpawnSettings> owned) && owned.Contains(settings)) return;
            if (Array.IndexOf(spawnSettings, settings) >= 0) return;
            if (pools.ContainsKey(settings)) throw new InvalidOperationException("다른 씬에서 사용하는 SpawnSettings입니다. 씬별 생성 설정을 사용하세요.");
            AddPool(settings, false, scene);
            if (owned == null) { owned = new HashSet<SpawnSettings>(); scenePools.Add(scene.handle, owned); }
            owned.Add(settings);
        }

        private void RemoveScenePools(Scene scene)
        {
            if (isTicking) throw new InvalidOperationException("Tick 종료 후 지역 풀을 제거해야 합니다.");
            if (!scenePools.TryGetValue(scene.handle, out HashSet<SpawnSettings> settings)) return;
            foreach (SpawnSettings setting in settings)
            {
                if (!pools.TryGetValue(setting, out WorldObjectPool pool)) continue;
                IReadOnlyList<WorldObjectView> views = pool.TakenViews;
                for (int j = views.Count - 1; j >= 0; j--)
                {
                    WorldObjectView view = views[j];
                    if (view == null) continue;
                    DisableView(view);
                    pool.DestroyTakenView(view);
                }
                pool.Dispose();
                pools.Remove(setting);
                for (int j = poolUsage.Count - 1; j >= 0; j--)
                    if (poolUsage[j].Settings == setting) poolUsage.RemoveAt(j);
            }
            scenePools.Remove(scene.handle);
            RefreshInspectorCounts();
        }
        ...
        private void DisableView(WorldObjectView view)
        {
        ...
        private void Shutdown()
        {
            ...
            foreach (KeyValuePair<SpawnSettings, WorldObjectPool> entry in pools)
            {
                IReadOnlyList<WorldObjectView> takenViews = entry.Value.TakenViews;
                for (int index = takenViews.Count - 1; index >= 0; index--)
                {
                    WorldObjectView view = takenViews[index];
                    if (view == null) continue;
                    DisableView(view);
