...
[ItemContainer](ItemContainer.cs)는 지역별 바닥 아이템의 수명을 맡을 빈 틀이다. 기존 ItemSpawnManager와 아직 연결하지 않았으며, 플레이어 인벤토리는 관리하지 않는다. [진입점과 이전 순서](../01.Boot/README.md)를 참고한다.

| 위치 | 설명 |
| --- | --- |
| `ItemDefinition.cs` | 표시 이름, 아이콘과 최대 묶음 수를 가진 ScriptableObject |
| `ItemType.cs`, `ItemCatalog.cs` | 아이템 종류와 정의·월드 프리팹의 연결 |
...
관련 문서: [플레이어](../10.Player/README.md), [교환·상호작용](../02.Core/WorldObjects/README.md).
