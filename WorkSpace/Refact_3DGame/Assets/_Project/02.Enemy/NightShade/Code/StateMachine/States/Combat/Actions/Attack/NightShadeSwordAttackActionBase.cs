...
namespace Characters.Enemies.NightShade
{
    ...
    {
        private const int EventQueueCapacity = 16;
        private float clipElapsedTime;
        private bool eventQueueOverflowed;
        ...
        public virtual bool ProtectsSmallHit => isActive;
        ...
        {
            if (UpdateAnimationTimeout(deltaTime))
                return;
            ProcessQueuedEvents();
            ...
            while (queuedEventCount > 0)
            {
                ...
                switch (pendingEvent.Type)
                {
                    ...
                    case AttackEventType.CloseHit:
                        // 한 프레임에 열기와 닫기가 모두 와도 판정 구간을 검사한다.
                        if (openHitIndex >= 0)
                            CombatOutput.CheckAttackHit();
                        CloseOpenHit();
                        break;
                }
            }
        ...
        protected bool UpdateAnimationTimeout(float deltaTime)
        {
            clipElapsedTime += Mathf.Max(0f, deltaTime);
            if (clipElapsedTime < 12f)
                return false;
            ClearQueuedEvents();
            CloseOpenHit();
            IsFinished = true;
        ...
        private void Enqueue(AttackEventType eventType, int hitIndex)
        {
            ...
            if (queuedEventCount >= EventQueueCapacity)
            {
        ...
        private void ProcessOpenHitEvent(int hitIndex)
        {
            ...
            // 피해 구간이 열린 뒤에는 플레이어를 따라 회전하지 않는다.
            canTurn = false;
            CloseOpenHit();
