...
namespace Characters.Enemies.NightShade
{
    ...
    internal sealed class NightShadeSwordStateMachine
    {
        ...
        internal bool NeedsBattleReset { get; private set; }
        ...
        internal void Update(float deltaTime, bool isHitStopActive = false)
        {
            ...
            if (battleSpace != null && hasBattleStarted && currentStateId != NightShadeSwordStateId.Dead)
            {
                targetLostTime = targetStatus.IsDetected ? 0f : targetLostTime + Mathf.Max(0f, deltaTime);
                NeedsBattleReset = !targetStatus.IsAlive || targetLostTime >= 2f ||
                    !battleSpace.Contains(movement.Position);
                if (NeedsBattleReset)
                    return;
            }
            if (isHitStopActive)
        ...
        internal void ProcessAnimationEvents()
        {
            if (IsAttackStateActive && !hasPendingDeath && pendingHitReaction == HitReaction.None)
                combatState.ProcessAnimationEvents();
        }
