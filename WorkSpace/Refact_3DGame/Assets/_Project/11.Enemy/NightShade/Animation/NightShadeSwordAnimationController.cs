...
namespace NightShade
{
    ...
    public sealed class NightShadeSwordAnimationController : MonoBehaviour, INightShadeSwordAnimation
    {
        ...
        private static readonly int[] RequiredBattleStates =
        {
            IdleStateId, ChaseStateId, WalkStateId, CombatBackStateId, CombatLeftStateId,
            CombatRightStateId, LightAttackStateId, ComboFirstStateId, ComboSecondStateId,
            HeavyAttackStateId, WideSwingStateId, SmallHitFrontStateId, SmallHitBackStateId,
            SmallHitLeftStateId, SmallHitRightStateId, HitFrontStateId, HitBackStateId,
            HitLeftStateId, HitRightStateId, StaggerEnterStateId, StaggerStartStateId,
            StaggerIdleStateId, StaggerEndStateId, DeadStateId
        };

        internal void ValidateBattleAnimations()
        {
            if (enemyAnimator == null || enemyAnimator.runtimeAnimatorController == null)
                throw new System.InvalidOperationException("NightShade Animator Controller가 연결되지 않았습니다.");
            for (int index = 0; index < RequiredBattleStates.Length; index++)
            {
                if (!enemyAnimator.HasState(0, RequiredBattleStates[index]))
                    throw new System.InvalidOperationException(
                        "NightShade Animator에 필수 전투 상태가 없습니다. 상태 해시: " + RequiredBattleStates[index]);
            }
        }
        ...
        internal bool IsRequestedAnimationEvent(AnimationEvent animationEvent)
        {
            return animationEvent != null &&
                animationEvent.animatorStateInfo.fullPathHash == requestedStateId;
        }
