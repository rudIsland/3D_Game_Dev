...
namespace NightShade
{
    internal sealed class NightShadeSwordCombatState : INightShadeSwordState
    {
        ...
        {
            if (actionRunner.CurrentAction is NightShadeSwordAttackActionBase attack)
                attack.ProcessQueuedEvents();
        }
        ...
        private NightShadeSwordStateId? UpdateApproach(float deltaTime)
        {
            ...
            bool isBlocked = context.Movement is NightShadeSwordMovement movement && movement.IsBlocked;
            if (isBlocked != approachBlocked)
            {
                approachBlocked = isBlocked;
                if (isBlocked)
                    context.Animation.PlayIdle();
                else if (approachMode == NightShadeSwordApproachMode.Walk)
