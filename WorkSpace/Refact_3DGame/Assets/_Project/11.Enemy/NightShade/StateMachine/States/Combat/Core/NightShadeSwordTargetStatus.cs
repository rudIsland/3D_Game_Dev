...
namespace NightShade
{
    internal sealed class NightShadeSwordTargetStatus
    {
        ...
        internal void Refresh()
        {
            ...
            bool canFightHere = battleSpace == null || battleSpace.Contains(TargetPosition);
            bool canSee = battleSpace == null || battleSpace.CanSee(TargetPosition);
            float detectionRangeSquared = IsDetected ? findRangeSquared * 1.5625f : findRangeSquared;
            IsDetected = canFightHere && distanceSquared <= detectionRangeSquared && (IsDetected || canSee);
            IsInsideAttackRange = IsDetected && distanceSquared <= attackRangeSquared && canSee &&
                (battleSpace == null || battleSpace.HasAttackHeight(TargetPosition));
