...
namespace Enemy
{
    ...
    public abstract class EnemyView : MonoBehaviour
    {
        ...
        {
            if (startFromScene && manager != null && RuntimeObject != null) manager.Enable(RuntimeObject);
        }

        protected virtual void OnDisable()
        {
            if (manager != null && RuntimeObject != null) manager.Disable(RuntimeObject);
        }

        protected virtual void OnDestroy()
        {
            // 씬이 직접 제거한 풀 객체도 등록 목록에서 빠져야 한다.
            if (manager != null && RuntimeObject != null) manager.Unregister(RuntimeObject);
        }
        ...
        {
            if (startFromScene) { gameObject.SetActive(false); return; }
