...
| 폴더 | 설명 |
| --- | --- |
| `Code/Lifecycle` | Unity 연결용 `ZombieController`와 실행용 `ZombieWorldUnit` |
| `Code/StateMachine/States` | 생존·피격·사망 상태와 상태 전환 |
| `Code/StateMachine/States/Alive` | 대기·경계·추격·공격·귀환 행동 |
| `Code/Movement` | CharacterController 이동과 경로 안내 연결 |
| `Code/Attack` | 공격 범위와 타격 판정 |
| `Code/Animation` | Animator 제어와 공격·경계 동작 이벤트 |
| `Code/Config`, `Configs` | 설정 타입, `ZombieConfig.asset`, `ZombieSpawnSettings.asset` |
...
관련 문서: [월드 객체·Zone](../../00.Core/WorldObjects/README.md), [적 공통](../Shared/README.md).
