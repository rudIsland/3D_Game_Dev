...
namespace Zombie
{
    ...
    public sealed class ZombieStateMachine
    {
        ...
        private readonly ZombieSettings settings;
        ...
        internal float FindRangeSquared => settings.FindRangeSquared; // 거리 설정
        internal float IdleTargetCheckInterval => settings.IdleTargetCheckInterval; // 대상 참조
        internal float AttackRangeSquared => settings.AttackRangeSquared; // 공격 관련 설정 또는 상태
        internal float ChaseSpeed => settings.ChaseSpeed; // 이동 속도
        internal float TurnSpeed => settings.TurnSpeed; // 이동 속도
        internal float HitPushDuration => settings.HitPushDuration;
        internal float KnockbackPushDuration => settings.KnockbackPushDuration;
        internal float DeadBodyKeepTime => settings.DeadBodyKeepTime; // 시간 설정
        ...
        internal bool CanTrackTarget()
        {
            return target != null &&
                ...
                (HomeZone == null ||
                    HomeZone.Contains(
                        target.position,
                        IsInCombat ? settings.ZoneTrackingMargin : 0f));
        }
        ...
        {
            return movement.IsFacing(targetPosition, settings.MinimumAttackFacingDot);
        }
        ...
        internal bool HasArrivedHome()
        {
            ...
            return homeDistance.sqrMagnitude <=
                settings.HomeArrivalDistanceSquared;
        }
        ...
        {
            return settings.EvaluateHitPushProgress(normalizedTime);
        }
