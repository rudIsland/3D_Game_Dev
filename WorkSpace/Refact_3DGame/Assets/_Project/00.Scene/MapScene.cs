    ...
    public sealed class MapScene : MonoBehaviour
    {
        ...
        public void Init()
        {
            if (playerSpawnPoint == null || playerSpawnPoint.gameObject.scene != gameObject.scene)
                throw new InvalidOperationException("MapScene의 playerSpawnPoint에 같은 씬의 시작 위치를 연결하세요.");
            if (ambientSource != null && (ambientSource.clip == null ||
                ...
                throw new InvalidOperationException("MapScene의 환경음은 같은 씬의 활성 AudioSource와 AudioClip을 연결하세요.");
        }
